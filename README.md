...
.card{border:1px solid #e8e8e8;border-radius:14px;overflow:hidden;background:#fff;transition:transform .25s,box-shadow .25s}.card:hover{transform:translateY(-5px);box-shadow:0 14px 35px #0000000d}.pic{position:relative;background:#f4f4f2;min-height:250px}.pic img{transition:transform .35s}.card:hover .pic img{transform:scale(1.035)}.tag{border-radius:999px!important}.featured-badge{border-radius:999px}.info{padding:15px}.info h3{min-height:38px}.row{gap:10px}.price{font-weight:800}
...
.detail-photo{border-radius:16px;overflow:hidden;background:#f5f5f3}.detail-photo img{display:block}.detail h1{font-size:clamp(32px,5vw,58px);letter-spacing:-.06em}.detail .btn{margin-top:20px}
...
@media(max-width:800px){header{height:64px;padding:0 18px}nav{gap:12px}nav button{font-size:10px}.hero{grid-template-columns:1fr;padding:55px 22px 45px;min-height:0}.hero h1{font-size:52px}.hero:after{font-size:100px}.section{padding-left:22px;padding-right:22px}.featured-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.pic{min-height:180px}.info h3{font-size:14px;min-height:0}.price{font-size:13px}.view{font-size:9px}.stat-grid{grid-template-columns:repeat(2,1fr)}.detail{grid-template-columns:1fr!important;gap:25px!important}.detail-photo{max-height:430px}.admin-table{font-size:11px}}
@media(max-width:430px){.featured-grid{grid-template-columns:1fr 1fr}.pic{min-height:150px}.featured-badge{font-size:7px;padding:6px 8px}.tag{font-size:8px}.controlbar{gap:8px}.controlbar .search{min-width:100%;}.controlbar select{width:100%}.hero h1{font-size:44px}.logo{font-size:25px}}
</style>
<script>
(function () {
...
<section class="section featured-section" id="featured"><div class="featured-head"><div><div class="featured-label">MOST RECOMMENDED / POPULAR FINDS</div><h2 style="margin-top:7px">Produk Terbaik</h2></div><button class="view" onclick="document.getElementById('products').scrollIntoView()">VIEW ALL →</button></div><div class="featured-grid" id="featuredGrid"></div></section>
...
function isSaved(id){return JSON.parse(localStorage.getItem('nebo_saved')||'[]').includes(id)}
function toggleSaved(id){const saved=JSON.parse(localStorage.getItem('nebo_saved')||'[]');const next=saved.includes(id)?saved.filter(x=>x!==id):[...saved,id];localStorage.setItem('nebo_saved',JSON.stringify(next));showToast(next.includes(id)?'Produk disimpan':'Produk dihapus dari simpanan');drawProducts();drawFeatured()}
...
function productCard(p,featured=false){const popular=(p.clicks||0)>=10;return `<article class="card featured-card">${featured?'<span class="featured-badge">RECOMMENDED</span>':popular?'<span class="featured-badge">POPULAR</span>':''}<div class="pic">${p.image?`<img src="${escapeAttr(p.image)}" loading="lazy" alt="${escapeAttr(p.name)}" style="width:100%;height:100%;object-fit:cover">`:'<span class="pictext">PRODUCT PHOTO</span>'}<span class="tag">${escapeHtml(p.category)}</span></div><div class="info"><div class="type">${escapeHtml(p.type)}</div><h3>${escapeHtml(p.name)}</h3><div class="row"><span class="price">${escapeHtml(p.price)}</span></div><div class="product-actions"><button class="view" onclick="openProduct(${p.id})">VIEW PRODUCT ↗</button><button class="heart-btn ${isSaved(p.id)?'saved':''}" aria-label="Simpan produk" onclick="toggleSaved(${p.id})">${isSaved(p.id)?'♥':'♡'}</button></div></div></article>`}
function drawFeatured(){const list=activeProducts().slice().sort((a,b)=>Number(b.featured)-Number(a.featured)||(b.clicks||0)-(a.clicks||0)||(b.createdAt||0)-(a.createdAt||0)).slice(0,4);document.getElementById('featuredGrid').innerHTML=list.length?list.map(p=>productCard(p,true)).join(''):'<div class="empty-state">Belum ada produk unggulan. Tambahkan produk melalui ADMIN.</div>'}
...
function openProduct(id){const p=products.find(x=>x.id===id);if(!p)return;document.getElementById('app').innerHTML=`<section class="section"><button class="view" onclick="showHome()">← BACK TO SHOP</button><div class="detail" style="margin-top:30px"><div class="detail-photo">${p.image?`<img src="${p.image}" style="width:100%;height:100%;object-fit:cover">`:'PRODUCT PHOTO'}</div><div><div class="eyebrow">${p.category} / ${p.type}</div><h1>${escapeHtml(p.name)}</h1><strong style="font-size:20px">${escapeHtml(p.price)}</strong><p class="desc">${escapeHtml(p.description)}</p><div class="product-actions"><button class="btn" onclick="affiliateClick(${p.id})">VIEW PRODUCT ↗</button><button class="heart-btn ${isSaved(p.id)?'saved':''}" onclick="toggleSaved(${p.id})">${isSaved(p.id)?'♥':'♡'}</button></div></div></div></section>`}
